  ...
  return (
    <div className="grid gap-section">
      <div className="grid gap-element">
        <h1 className="text-loud">Re-derive a receipt</h1>
        <p className="max-w-3xl text-plain">
      ...
      <section className="grid gap-element lg:grid-cols-2">
        <div className="grid gap-3">
          <label
            ...
            className="text-caption uppercase tracking-[0.14em] text-quiet"
          >
            Receipt JSON
          </label>
          <div
          ...
          >
            <textarea
              ...
              className="num w-full resize-y rounded-default bg-ink-sunken px-3 py-3 text-caption leading-relaxed text-plain placeholder:text-quiet"
            />
          </div>
        </div>
        ...
        <div className="grid gap-3">
          ...
          <textarea
            ...
            className="num w-full resize-y rounded-default border border-rule bg-ink-sunken px-3 py-3 text-caption leading-relaxed text-plain placeholder:text-quiet"
          />
        </div>
      </section>

      <div className="flex flex-wrap gap-2">
        <Button variant="primary" onClick={run} disabled={text.trim().length === 0}>
          Re-derive
        </Button>
        <Button
          onClick={() => {
      ...
      {outcome.kind === "invalid" ? (
        <Callout>
          <p className="num text-loud">This input is not a receipt.</p>
          <ul className="num mt-2 space-y-1 text-plain">
      ...
      {outcome.kind === "derived" ? (
        <section className="grid gap-element">
          <div
            className={`rounded-featured border p-element ${
              outcome.result.ok ? "border-rule bg-ink-raised" : "border-accent bg-accent-quiet"
            }`}
          >
            <p className="num text-caption uppercase tracking-[0.14em] text-quiet">
              {outcome.result.ok ? "Re-derives" : "Does not re-derive"}
            </p>
            <p className="num mt-3 text-loud">
              {outcome.result.ok
            ...
            {outcome.result.problems.length > 0 ? (
              <ul className="num mt-3 space-y-1 text-plain">
          ...
          <Card>
            ...
            <dl className="mt-3 grid gap-2">
              <div className="border-b border-rule pb-2">
                ...
                <dd className="num mt-1 break-all text-plain">{outcome.result.receiptLeaf}</dd>
              </div>
              <div>
                <dt className="text-caption uppercase tracking-[0.12em] text-quiet">
                  Response hash commitment
                </dt>
                <dd className="num mt-1 text-plain">
      ...
      <Callout>
        <p className="text-loud">What this check does and does not establish.</p>
        <p className="mt-2">
